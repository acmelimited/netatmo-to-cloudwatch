...
def fetch_weather_data():   
    ...
    # You will need a client id and client secret from Netatmo which can be obtained when you register
    # your app with them https://dev.netatmo.com/apps/createanapp#form
    ssm = boto3.client('ssm')
    secrets = ssm.get_parameters(Names=['Netatmo_Client_Id', 'Netatmo_Username', 'Netatmo_Client_Secret', 'Netatmo_Password'], WithDecryption=True)
    ...
    for station_key in weather_data.stations:
        ...
        metric_data.append(create_metric_data("Temperature", station_name, station_temp, station_time))
        
        station_co2 = get_dashboard_data("CO2", station)
        metric_data.append(create_metric_data("CO2", station_name, station_co2, station_time))

        station_humidity = get_dashboard_data("Humidity", station) 
        metric_data.append(create_metric_data("Humidity", station_name, station_humidity, station_time))
        ...
        metric_data.append(create_metric_data("Air_Pressure", station_name, station_pressure, station_time))
        ...
        for module in station["modules"]:
            ...
            # The rf_status is the signal strength. Please refer to https://dev.netatmo.com/apidocumentation/weather 
            # for an explaination of the values.
            module_signal = get_data("rf_status", module)
            metric_data.append(create_metric_data("Signal_Strength", module_name, module_signal, module_time))
            ...
            # Please refer to https://dev.netatmo.com/apidocumentation/weather for an explaination of the values.
            module_battery = get_data("battery_vp", module)
            metric_data.append(create_metric_data("Battery_Status", module_name, module_battery, module_time))
            ...
            if "Temperature" in module["data_type"]:
                ...
                metric_data.append(create_metric_data("Temperature", module_name, module_temperature, module_time))

                module_humidity = get_dashboard_data("Humidity", module)
                metric_data.append(create_metric_data("Humidity", module_name, module_humidity, module_time))
            ...
            if "Rain" in module["data_type"]:
                ...
                metric_data.append(create_metric_data("Rain", module_name, module_rain, module_time))

                module_rain_1hour = get_dashboard_data("sum_rain_1", module)
                metric_data.append(create_metric_data("Rain_1_hour", module_name, module_rain_1hour, module_time))

                module_rain_24hour = get_dashboard_data("sum_rain_24", module)
                metric_data.append(create_metric_data("Rain_24_hours", module_name, module_rain_24hour, module_time))
